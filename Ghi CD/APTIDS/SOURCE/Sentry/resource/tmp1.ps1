...
This parameter is optional and will specify the duration, in minutes, that traffic will be collected before analysis is performed. If no value is specified, then the network trace will run for 5 minutes by default.

.PARAMETER Folder

This parameter is optional and will specify the folder where the packet capture will be stored. This is useful if the user wants to export and convert the resulting event trace file to .pcap format using Microsoft Message Analyzer. If no value is specified, then the script will use the folder C:\temp
...
#> 
	Param(
		...
		[Parameter(Position = 2, Mandatory = $false)] [int]
		$Duration = 5,
		[Parameter(Position = 3, Mandatory = $false)] [int]
		$Size = 250
	)
...
This module performs a check of the layer 2 cache on the local computer to determine whether addresses of interest are cached. Given the frequency with which the interesting protocols communicate, it is likely that the presence of these cached
...
identify whether multicast addresses for a given protocol are present. If so, the output reports the presence of the protocol and which OSI layer it was observed at.
...
# netsh output and parse results $parseOld = $false
	try
	{
...
in Windows 8 and above. It does not return NetNeighbor objects. Only the information (MAC and IP address) returned from the netsh commands that are used within the functionality of the exposed commandlets in this package.
...
netsh int ipv4 show neigh interface=$InterfaceIndex netsh int ipv6 show neigh interface=$InterfaceIndex
...
# Write the output to the pipeline Write-Output $neighbor
			}
		}
	}
}
...
#	This may need to be changed to support a computer with multiple interfaces 
	...
	if($firewall_status)
	{
		$analyzer.console_queue.Add("Windows Firewall = Enabled1") > $null 
...
#	Shared Basic Functions ScriptBlock 
	...
	$sniffer_scriptblock =
	{
		param ($IP,$RunTime)
		Write-Host "Debug"
		Write-Host "132" | Out-File ".\\test.log"
							...
							# Handle OSPF Packets with NULL Auth 
								0
								{
									switch($ospf_type)
									{
										...
										5
										{
											$analyzer.console_queue.Add(" [i] Type: LS Ackpacket.") > $null
										}
									}
									$analyzer.console_queue.Add(" [!] Auth: NULL") > $null
								}
...
# Moved sniffer to main script instead of function so thread can be properly shut down 
	$test = {
		param ($IP,$RunTime)
		Write-Host "TESST ZZZZZZZZZZZZZZZ"
	}
	...
	Write-Host $sniffer_powershell
	$sniffer_powershell.Runspace = $sniffer_runspace
	$sniffer_powershell.AddScript($test)
	$sniffer_powershell.AddScript($shared_basic_functions_scriptblock)
	$sniffer_powershell.AddScript($sniffer_scriptblock).AddArgument($IP).AddArgument($RunTime)
	$sniffer_powershell.BeginInvoke()
	Write-Host $sniffer_powershell
	while ($analyzer.running -or ($analyzer.console_queue.Count -gt 0))
	{
		...
		if( [ Console]::KeyAvailable)
		{
			...
			switch ($key.KeyChar)
			{
				...
				# Set analyzer to stopped and reset show variables $analyzer.running = $false
					$analyzer.show_dhcp = $true 
					$analyzer.show_hsrp = $true 
					$analyzer.show_llmnr = $true 
					$analyzer.show_mdns = $true 
				...
				# Kill the sniffer objects $sniffer_powershell.Dispose() $sniffer_runspace.CloseAsync() $sniffer_runspace.Dispose()
